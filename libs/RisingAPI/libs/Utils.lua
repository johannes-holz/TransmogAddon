...
local afterLoginFrame = CreateFrame("Frame")
local afterLoginQueue = {}
afterLoginFrame:SetScript("OnEvent", function()
	for _, func in ipairs(afterLoginQueue) do
		xpcall(func, geterrorhandler())
	end
	afterLoginQueue = nil
end)
afterLoginFrame:RegisterEvent("PLAYER_LOGIN")

function Utils.DoAfterLogin(func)
	if (afterLoginQueue) then
		table.insert(afterLoginQueue, func)
	else
		func()
	end
end
