...
-- TODO: Unsure if we want to bother with modifying this function. Probably needs fixes to interact with other addons, even Blizzard Auction UI already overwrites this.
...
DressUpModel.SetSlot = function(self, itemSlot, itemID, silent)
    ...
    if itemID then
        ...
        if itemID > 1 and itemSlot == "ShieldHandWeaponSlot" then
            local itemSubType, _, itemEquipLoc = select(7, GetItemInfo(itemID))
